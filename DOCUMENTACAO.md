...
Ordem de saída: `fatura_resumida` → `fatura` → `cliente` → `itens_fatura` →
`impostos` → `medicao` → `medicao_resumida` → `glossario` (+ aba oculta
`_faturas_meta` com metadados).

| Aba | Conteúdo | Origem |
|---|---|---|
| `fatura` | 1 linha/fatura: id, datas, valor, dados fiscais, classificação, SCEE, leituras | processador |
| `cliente` | dados da UC (razão social, CNPJ, endereço) | processador |
...
| `fatura_resumida` | **1ª aba**; subconjunto de `fatura` | **derivada** de `fatura` |
...
- **`id_uc`** aparece em TODAS as abas; quando a fatura não traz UC, recebe
  ...
  usado também pela CHESP). **`competencia`** aparece em todas as abas exceto `cliente`.
- **`cliente.ultima_competencia` / `ultima_fatura`**: agregados por `id_uc`,
  recalculados por `derivados.py` no processamento e na concatenação (do zero).
- **`id_uc_normalizado`** (ao lado de `id_uc` em todas as abas): por medidor, o
  id_uc mais recente e não-`NULO_`. **`item_normalizado`** (em `itens_fatura`):
  vem da tabela de equivalências (`equivalencias.py`, aba Parâmetros). Ambos são
  recalculados do zero em `derivados.aplicar` / `aplicar_concat`.
...
│   ├── derivados.py     colunas recalculadas do zero: cliente.ultima_*, id_uc_normalizado, item_normalizado
...
1. `controller.processar_jobs` percorre as pastas; para cada PDF chama
   ...
   as linhas de cada aba base (`fatura`, `cliente`, `itens_fatura`, `impostos`,
   `medicao`). Roda em **thread** (`gui/worker.py`); a GUI faz polling da fila.
